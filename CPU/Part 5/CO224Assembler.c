...
This simple assembler assumes an ISA containing the following instructions: loadi, mov, add, sub, and, or, j, beq, lwd, lwi, sll, srl. 
All instructions are encoded into 32-bit words based on the following format:

Bits 31-24 : OP-CODE				: Given as one of (loadi, mov, add, sub, and, or, j, beq, lwd, lwi, sll, srl)
	...
	*************************************************************************/
 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*************************************************************************/
		...
		strcasecmp(in_token,"sll")==0 || strcasecmp(in_token,"srl")==0 || strcasecmp(in_token,"j")==0) 
		{
			int j = (strcasecmp(in_token,"j")==0)?1:0; //Flag if this is a 'j' (j is the only instruction ignoring bits 7-0)
            int store = (strcasecmp(in_token,"sll")==0 || strcasecmp(in_token,"srl")==0)?1:0; //Flag if this is a store instruction
